...
if __name__ == "__main__":
    ...
    while True:
        ...
        #         break
            ...
            # utterance_to_rewrite = ["小王，你找我来干啥？", "李工，想让你帮我看看这个设计是不是合理。"]
            # generated_rewrite, generated_response = generate_example(utterance_to_rewrite, generation_args, prefix_model, label_embeds,
            ...
            # reduced_generated_response.extend(post_processing(generated_rewrite))
        ...
        if audio_message and audio_message['type'] == 'asr_result':
            ...
            if audio_message['speaker'] == 'FLE':
                context_FLE.append(audio_message['asr_text'])
            else:
                context_SME.append(audio_message['asr_text'])
        ...
        if rewrite_message and rewrite_message['type'] == 'offensive_speech':
            ...
            if len(context_FLE) >= 1:
                utterance_to_rewrite_list = [context_FLE[-1], rewrite_message['input_text']]
            else:
                utterance_to_rewrite_list = [rewrite_message['input_text'], rewrite_message['input_text']]
            generated_rewrite, generated_response = generate_example(utterance_to_rewrite_list, generation_args, prefix_model, label_embeds,
                                                  config,
